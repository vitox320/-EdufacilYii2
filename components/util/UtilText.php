...
//        if (isset($objectException->errorInfo) && !empty($objectException->errorInfo) && count($objectException->errorInfo) > 0) {
//            return 'Ocorreu erro(s) em sua requisição!<br/><br/>'
//                . '<a class="btn btn-sm btn-primary" role="button" data-toggle="collapse" href="#collapseExample" aria-expanded="false" aria-controls="collapseExample">'
//                . '<span class="glyphicon glyphicon-th-list" aria-hidden="true"></span><b> Detalhes Técnico</b>'
//                . '</a>'
//                . '<div class="collapse" id="collapseExample">'
...
//                . '</div>';
//        } else {
//            return  $objectException->getMessage();
//        }
    ...
    public static function emailParcial($email, $qtdCaracteres = 2) {
    ...
    /* Exemplo dos parametros $emails, $emailsComCopia, $emailsComCopiaOculta
     * $emails = $emailsComCopia = $emailsComCopiaOculta = array(
     *      "carla@email.com" => "Carla",
     *      "carlos@email.com" => "Carlos"
     * )
     */

    public static function enviarEmail($subject, $emails, $body, $emailsComCopia = array(), $emailsComCopiaOculta = array()) {
        $mail = new PHPMailer();
        $mail->IsSMTP();
        $mail->IsHTML(true);
//        $mail->Encoding;
...
//        $mail->Encoding = "16bit";
        try {
            if (\Yii::$app->params['environment'] === 'prod') {
                $mail->Host = "smtp.salvador.ba.gov.br";
                $mail->From = 'nao-responder@salvador.ba.gov.br';
            } else { // YII_ENV_HOM || YII_ENV_DEV
                $mail->Host = 'smtp.gmail.com';
                $mail->From = 'nao-responder@salvador.ba.gov.br';
//            $mail->From = 'nao-responder.transalvador@salvador.ba.gov.br';
//            $mail->setFrom('nao-responderSGCInforme@salvador.ba.gov.br', 'SGCInforme');
//            $mail->addReplyTo('nao-responderSGCInforme@salvador.ba.gov.br');

                $mail->SMTPSecure = "ssl";
                $mail->SMTPDebug = false;
                $mail->SMTPAuth = true;
                $mail->Port = 465;
                $mail->Username = 'assti.transalvador@gmail.com';
                $mail->Password = '#tr4ns4lv4d0r!!';
            }

//            $emails = array("kprofissional@gmail.com" => "Quemuel Leal");

            foreach ($emails as $email => $nome) {
                $mail->AddAddress($email, $nome);
            }

            foreach ($emailsComCopia as $email => $nome) {
                $mail->AddCC($email, $nome);
            }

            foreach ($emailsComCopiaOculta as $email => $nome) {
                $mail->AddBCC($email, $nome);
            }

            $mail->FromName = 'TRANSALVADOR';
            $mail->Subject = $subject;

            $rodape = "<br/><br/><br/><br/><img style='width: 210px;' alt='Transalvador' src='http://www.salvador.ba.gov.br/Logotipos_Cabecalho/logo_cabecalho/transalvador.png" . "'><br/>"
                    . "<br/>Atenciosamente,"
                    . "<br/>Superintedência de Trânsito de Salvador - Transalvador"
                    . "<br/><a href='http://www.transalvador.salvador.ba.gov.br' target='_blank'>www.transalvador.salvador.ba.gov.br</a>";

            $mail->Body = $body . $rodape;

            $enviado = $mail->Send();

//            if (!$enviado) {
//                throw new phpmailerException("Erro no envio de email");
//            }

            $mail->ClearAllRecipients();

            return array('valid' => true, 'message' => "Enviado com sucesso");
        } catch (phpmailerException $e) {
            return array('valid' => false, 'message' => $e->errorMessage());
        } catch (Exception $e) {
            return array('valid' => false, 'message' => $e->errorMessage());
        }
    }
